...
Output: otto_demo.snap_synergy.batch_synergy_scores
"""
import os
...
# Load cluster assignments from centroids table
...
pairs = (
    a.crossJoin(b)
    .filter(F.col("a.card_name") < F.col("b.card_name"))  # avoid duplicates + self-pairs
    .select(
...
scored = pairs.withColumn(
    # Same archetype bonus (0.3)
    "same_archetype",
    F.when(F.col("arch_a") == F.col("arch_b"), 0.3).otherwise(0.0),
).withColumn(
    # Shared tag bonus: count overlapping tags
    "tag_overlap",
    (
    ...
    # Cost curve bonus: different costs = better (can play together)
    "cost_curve",
    ...
    # Compute shared tags string for display
    "shared_tags",
    F.concat_ws(
        ",",
...
# Select final output — create both directions for easy lookup
forward = scored.select(
    F.col("card_a"), F.col("card_b"),
    F.col("synergy_score"), F.col("shared_tags"), F.col("same_cluster"),
)
...
print(f"  Primary key: (card_a, card_b) — ready for Lakebase sync")
