...
chatRouter.post('/', requireAuth, async (req: Request, res: Response) => {
  // === PLAIN JSON FORMAT HANDLER ===
  // Handles two plain-JSON formats (neither uses the Vercel SDK wire protocol):
  //   1. Array message format:  { messages: [{role, content}, ...] }
  //   2. Flat text format:      { message: "text" }
  if (Array.isArray(req.body.messages) || typeof req.body.message === 'string') {
    let coreMessages: ModelMessage[];

    if (Array.isArray(req.body.messages)) {
      // Array message format — full history sent by the client
      const incoming = req.body.messages as Array<{ role: string; content: string }>;
      coreMessages = incoming.map((m) => ({
        role: m.role as 'user' | 'assistant' | 'system',
        content: m.content,
      }));
    } else {
      // Flat text format — only the current message, no history
      coreMessages = [{ role: 'user' as const, content: req.body.message as string }];
    }
    ...
    // Emit debug payload so client can inspect what the backend sends to the LLM
    res.write(
      `data: ${JSON.stringify({ type: 'debug', payload: coreMessages })}\n\n`,
    );

    try {
      const model = await myProvider.languageModel('chat-model');
      const result = streamText({ model, messages: coreMessages });
      ...
      console.error('[Chat flat] Error:', error);
  ...
  // === VERCEL AI SDK FORMAT (existing handler below) ===
  ...
  try {
    ...
    const stream = createUIMessageStream({
      ...
      execute: async ({ writer }) => {
        // Emit debug payload so client can inspect the exact messages array sent to the LLM
        writer.write({ type: 'data-debugPayload', data: modelMessages });
